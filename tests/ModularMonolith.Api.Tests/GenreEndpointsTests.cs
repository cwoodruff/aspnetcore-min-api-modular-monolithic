...
public class GenreEndpointsTests(WebApplicationFactory<Program> factory)
    : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly WebApplicationFactory<Program> _factory = factory.WithWebHostBuilder(_ => { });

    [Fact]
    public async Task GetGenreById_ShouldReturn401_WhenNoToken()
    {
    ...
    [Fact]
    public async Task GetGenreById_ShouldReturn200Shape_WhenAuthorized()
    {
    ...
    [Fact]
    public async Task GetGenres_ShouldReturn200_WhenAuthorized()
    {
        var tenantFactory = _factory.WithAdminTenantUser();
        var client = tenantFactory.CreateClient();
        var token = await TestAuthHelpers.GetAccessTokenAsync(client);
