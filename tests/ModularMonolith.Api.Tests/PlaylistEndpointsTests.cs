...
public class PlaylistEndpointsTests(WebApplicationFactory<Program> factory)
    : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task GetPlaylistById_ShouldReturn200Shape_WhenAuthorized()
    {
    ...
    [Fact]
    public async Task GetPlaylists_ShouldReturn200_WhenAuthorized()
    {
        var tenantFactory = _factory.WithTenantUser();
        var client = tenantFactory.CreateClient();
        var token = await TestAuthHelpers.GetAccessTokenAsync(client);
