...
- 2026-07-19T08:24:52-04:00: Diagnosed the `/api/admin/customers` 403 as split/incomplete `Identity:InMemoryUsers` configuration, added startup warnings for ignored user entries, documented exact admin claim requirements, and added regression coverage for successful admin authorization.
- 2026-07-19T09:37:25-04:00: Fixed the NSubstitute 6 test-only nullable break after a manual NuGet upgrade by adding the shared `TestCacheKeys` helper and explicit null checks in `Arg.Is(...)` matchers; build returned to 0 errors and the solution test run stayed green at 244/244.
