...
public sealed class HttpContextTenantResolutionService : ITenantResolutionService
{
    ...
    public string? ResolveTenantId(HttpContext httpContext)
    {
        ...
        if (httpContext.Request.RouteValues.TryGetValue("tenant", out var t) && t is string s1 && !string.IsNullOrWhiteSpace(s1))
            return s1;
        if (httpContext.Request.RouteValues.TryGetValue("tenantId", out var t2) && t2 is string s2 && !string.IsNullOrWhiteSpace(s2))
            return s2;
